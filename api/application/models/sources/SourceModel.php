...
class SourceModel extends CI_Model {
    ...
    public function getSourceList() {
        if ($this->session->user_type == 'CLIENT') {
           $query= "SELECT `a`.*, `b`.`name` as `username`, `d`.`project_name` as `project_name`, `c`.`pa`, `c`.`da`, `c`.`moz_rank`, `c`.`citation_flow`, `c`.`trust_flow`, `c`.`category` FROM `source` as `a` LEFT JOIN `users` as `b` ON `a`.`user_id` = `b`.`id` LEFT JOIN `seo_data` as `c` ON `a`.`id` = `c`.`source_id` LEFT JOIN `project_details` as `d` ON `a`.`project_id` = `d`.`id` WHERE `a`.`status` = 'TRUE' AND (`c`.`status`='TRUE' OR `c`.`status` is null) AND `a`.`user_id`='".$this->session->user_id."'";     
        }else{
            $query= "SELECT `a`.*, `b`.`name` as `username`, `d`.`project_name` as `project_name`, `c`.`pa`, `c`.`da`, `c`.`moz_rank`, `c`.`citation_flow`, `c`.`trust_flow`, `c`.`category` FROM `source` as `a` LEFT JOIN `users` as `b` ON `a`.`user_id` = `b`.`id` LEFT JOIN `seo_data` as `c` ON `a`.`id` = `c`.`source_id` LEFT JOIN `project_details` as `d` ON `a`.`project_id` = `d`.`id` WHERE `a`.`status` = 'TRUE' AND (`c`.`status`='TRUE' OR `c`.`status` is null)";     
        }
